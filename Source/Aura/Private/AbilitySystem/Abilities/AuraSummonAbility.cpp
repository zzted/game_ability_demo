...
TArray<FVector> UAuraSummonAbility::GetSpawnLocations() const
{
	...
	{
		const FVector Direction = RightOfSpread.RotateAngleAxis(i * DeltaSpread, FVector::UpVector);
...
{
	const int32 MinionIndex = FMath::RandRange(0, MinionClasses.Num() - 1);
	return MinionClasses[MinionIndex];
}
