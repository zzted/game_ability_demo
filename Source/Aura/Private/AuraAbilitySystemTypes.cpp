...
bool FAuraGameplayEffectContext::NetSerialize(FArchive& Ar, class UPackageMap* Map, bool& bOutSuccess)
{
	...
	if (RepBits & (1 << 7))
	{
		Ar << bIsBlockedHit; // not necessary
		bIsBlockedHit = true;
	}
	else
	{
	...
	if (RepBits & (1 << 8))
	{
		Ar << bIsCriticalHit; // not necessary
		bIsCriticalHit = true;
	}
	else
	{
	...
	if (RepBits & (1 << 9))
	{
		Ar << bIsSuccessfulDebuff; // not necessary
	...
	if (RepBits & (1 << 14))
	{
		DeathImpulse.NetSerialize(Ar, Map, bOutSuccess); // FVector has inherited serialize function override
	}
	if (RepBits & (1 << 15))
	{
